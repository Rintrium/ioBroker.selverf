...
class SelveUSBGateway {
	 ...
	 * @param {*} expectedSplitIDArray Array of splitID Arrays of strings
	 * @param {*} actuatorID Array of actuatorIDs corresponding to the Array of splitIDs
	 * @param {*} splitIDChecks int array that defines which parts of the splitID are checked for a match
	 */
	ClearCommandFromSendingQueue(expectedSplitIDArray, splitIDChecks, actuatorID)
	{
		let activeSplitID;
		let splitIDMatch = true;
		for (let i = 0; i < expectedSplitIDArray.length; i++)
		{
			activeSplitID = expectedSplitIDArray[i];
			for (let j = 0; j < this.messagesSending.length; j++)
			{
				splitIDMatch = true;
				for (let k = 0; k < splitIDChecks.length; k++)
				{
					if (this.messagesSending[j].splitID[splitIDChecks[k]] != activeSplitID[splitIDChecks[k]]) splitIDMatch = false;
				}

				if (splitIDMatch && this.messagesSending[j].identifier == actuatorID[i]) //there is a match, the command can be removed or at least marked as cleared once
				{
		...
		{
			this.isSending = false;
			if (this.queuedMessages.length > 0) this.SendQueuedMessages();
		}		
	}
