		...
		if (error != null)
		{
			...
			if (this.activeRetry > 0)
			{
				this.adapter.terminate(
					...
					utils.EXIT_CODES.ADAPTER_REQUESTED_TERMINATION, TODO
				);
			}
	...
	GenerateGatewayMessage()
	{
		...
		if (this.messagesSending[0].splitID[2] == "actuator")
		{
			...
			else if (this.messagesSending[0].splitID[3] == "iveo")
			{
				...
				switch (this.messagesSending[0].splitID[5]) {
					case "up":
						iveoCommand = COMMEO_COMMAND_DRIVEUP;
						break;

					case "down":
						iveoCommand = COMMEO_COMMAND_DRIVEDOWN;
						break;

					case "stop":
						iveoCommand = COMMEO_COMMAND_STOP;
						break;

					case "drivePos1":
						iveoCommand = COMMEO_COMMAND_DRIVEPOS1;
						break;

					case "drivePos2":
						iveoCommand = COMMEO_COMMAND_DRIVEPOS2;
						break;
