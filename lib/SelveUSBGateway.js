...
class SelveUSBGateway {
	...
	ConnectUSBGateway()
	{
		...
		try {
			this.gatewayPort = new SerialPort({ path: this.adapter.config.usbGatewayPath, baudRate: 115200, lock: true}, this.ConnectionResultHandler.bind(this));
	...
	async ProcessGatewayMessage(data)
	{
		...
		if (data["methodResponse"])
		{
			...
			else
			{
				...
				else if (data["methodResponse"]["array"][0]["string"] == "selve.GW.sender.getIDs")
				{
					...
					this.ClearCommandFromSendingQueue(["inte","rnal","gateway","requestSenderIDs"], [0,1,2,3], [0]);

					let numOfSenders = 0;
					for (let i = 0; i < 63; i++) //only 63 senders possible
					{
						if (bArray[i])
						{
							numOfSenders++;
				...
				{
					this.ClearCommandFromSendingQueue(["inte","rnal","gateway","requestSenderInfo"], [0,1,2,3], [parseInt(data["methodResponse"]["array"][0]["int"][0])]);
				...
				{
					this.ClearCommandFromSendingQueue(["inte","rnal","gateway","requestSenderValues"], [0,1,2,3], [parseInt(data["methodResponse"]["array"][0]["int"][0])]);
				...
				else if (data["methodResponse"]["array"][0]["string"][0] == "selve.GW.command.device")
				{
					...
					{ //no error handling necessary. command will not be cleared an thus resent
						this.adapter.log.debug("Failed to move single actuator (commeo). Message will not be cleared from queue and may be resent.");
					}
				}
				else if (data["methodResponse"]["array"][0]["string"][0] == "selve.GW.iveo.commandManual")
				{
					...
					{ //no error handling necessary. command will not be cleared an thus resent
						this.adapter.log.debug("Failed to move single actuator (iveo). Message will not be cleared from queue and may be resent.");
					}
				}
		...
		else if (data["methodCall"])
		{
			...
			else if (data["methodCall"]["methodName"] == "selve.GW.command.result")
			{
				...
				if (success == 0) //there is at least one actuator, that did not execute the command
				{ //
					const bFailedArray = this.DecodeBase64ToBoolArray(maskFailed);
					for (let i = 0; i < 64; i++) if (bFailedArray[i]) this.adapter.log.debug("Failed to move actuator with id " + i.toString() + ". Message will not be cleared from queue and may be resent.");
				}
	...
	HandleCommeoActuatorValues(actuatorID, actuatorName, status, value, targetValue)
	{
		...
		this.adapter.setStateAsync(COMMEO_ACTUATOR_STATE_PREFIX + "." + actuatorID + ".scaledPositionInverted",
			{ val: (100 - Math.max(0, Math.min(100, Math.round(parseInt(value)/65535*100)))), ack: true });
		this.adapter.setStateAsync(COMMEO_ACTUATOR_STATE_PREFIX + "." + actuatorID + ".scaledTargetPosition",
			{ val: (Math.max(0, Math.min(100, Math.round(parseInt(targetValue)/65535*100)))), ack: true });
		this.adapter.setStateAsync(COMMEO_ACTUATOR_STATE_PREFIX + "." + actuatorID + ".scaledTargetPositionInverted",
			{ val: (100 - Math.max(0, Math.min(100, Math.round(parseInt(targetValue)/65535*100)))), ack: true });
	}
	//#endregion
	...
	{
		this.adapter.setStateAsync(SENSOR_STATE_PREFIX + "." + senderID + ".name",
			{ val: senderName, ack: true });
	}

	HandleSenderValues(senderID, lastEventNumerical)
	{
		this.adapter.setStateAsync(SENDER_STATE_PREFIX + "." + senderID + ".lastEventNumerical",
			{ val: parseInt(lastEventNumerical), ack: true });
		this.adapter.setStateAsync(SENDER_STATE_PREFIX + "." + senderID + ".lastEventNumerical",
			{ val: SENDER_EVENTS[parseInt(lastEventNumerical)], ack: true });
	...
	HandleSubscribedStateChange(id, state)
	{
		...
		if (splitID[2] == "actuator" && splitID[3] == "commeo" && (splitID[5] == "scaledTargetPosition" || splitID[5] == "scaledTargetPositionInverted")) //translation into targetPosition
		{
			...
			else //must be inverted
			{
				splitID[5] = "targetPosition";
				this.HandleInternalCommand(splitID, (65535 - Math.max(0, Math.min(65535, Math.round(state.val/100*65535)))));
			}
		}
	...
	GenerateGatewayMessage()
	{
		...
		else if (this.messagesSending[0].splitID[2] == "gateway")
		{
			...
			{
				this.messagesSending[0].PrepareForSendingQueue(false, this.messagesSending[0].val);
				return "<methodCall><methodName>selve.GW.sensor.getInfo</methodName><array><int>" + this.messagesSending[0].val + "</int></array></methodCall>";
			}
			...
			{
				this.messagesSending[0].PrepareForSendingQueue(false, this.messagesSending[0].val);
				return "<methodCall><methodName>selve.GW.sender.getValues</methodName><array><int>" + this.messagesSending[0].val + "</int></array></methodCall>";
			}
			else if (this.messagesSending[0].splitID[3] == "requestSenderInfo")
			{
				this.messagesSending[0].PrepareForSendingQueue(false, this.messagesSending[0].val);
				return "<methodCall><methodName>selve.GW.sender.getInfo</methodName><array><int>" + this.messagesSending[0].val + "</int></array></methodCall>";
			}
