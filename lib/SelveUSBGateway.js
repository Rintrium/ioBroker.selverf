...
class SelveUSBGateway {
	...
	async ProcessGatewayMessage(data)
	{
		...
		else if (data["methodCall"])
		{
			...
			{
				let success = data["methodCall"]["array"][0]["int"][0];
				let maskSuccess = data["methodCall"]["array"][0]["base64"][0];
				let maskFailed = data["methodCall"]["array"][0]["base64"][1];

				if (success == 1) //no errors while executing command
				{
					let bArray = this.DecodeBase64ToBoolArray(maskSuccess);
	...
	LostConnectionHandler()
	{
		...
		//or the possibility to resend messages
	}
}
