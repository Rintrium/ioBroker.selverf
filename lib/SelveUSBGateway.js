	...
	async ProcessGatewayMessage(data)
	{
		...
		else if (data["methodCall"])
		{
			...
			else if (data["methodCall"]["methodName"] == "selve.GW.event.sender")
			{
				...
				if (event >= 0 && event <= 13) 
				{
					this.adapter.log.info("Sender " + senderName + " with ID: " + senderID + ", reports " + SENDER_EVENTS[event]);
				}
				else 
				{
					this.adapter.log.warn("Sender " + senderName + " with ID: " + senderID + ", reports an unkown event (" + event + "): " + JSON.stringify(data));
				}
			}
			else
			{
