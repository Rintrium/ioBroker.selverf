...
const CONNECTION_TIMEOUT_DURATION = 800; //ms
...
//TODO berücksichtigen des Anfangs der Nachricht! Alles davor verwerfen
	...
	async ProcessGatewayMessage(data)
	{
		...
		if (data["methodResponse"])
		{
			if (data["methodResponse"]["fault"]) //every other method response has to clear sendingMessages
			{
				...
				this.adapter.log.info("Resending last message!");
	...
	{
		this.queuedMessages.unshift(new GatewayCommand(splitID, val));
	...
	RetryHandler()
	{
		...
		{
			this.adapter.log.console.warn("Trying to reestablish connection with gateway. Try number " + this.activeRetry);
