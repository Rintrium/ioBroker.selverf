...
class SelveUSBGateway {
	...
	ConnectUSBGateway()
	{
		...
		try {
			this.gatewayPort = new SerialPort(this.adapter.config.usbPath, { baudRate: 115200 }, this.ConnectionResultHandler.bind(this));

		} catch (error) {
			this.adapter.log.error("Invalid port: " + this.adapter.config.usbPath + "; Error: " + error);
